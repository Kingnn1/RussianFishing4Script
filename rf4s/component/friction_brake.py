...
class FrictionBrake:
    ...
    def reset(self, target: int) -> None:
        ...
        with self.lock:
            sleep(add_jitter(FRICTION_BRAKE_CHANGE_DELAY))  # might be called after change()
    ...
    try:
        while True:
            if not friction_brake.detection.is_fish_hooked_pixel():
                ...
                continue
            if not fish_hooked:
                sleep(add_jitter(friction_brake.fb_cfg.START_DELAY, friction_brake.cfg.BOT.JITTER))
                fish_hooked = True
            ...
            with friction_brake.lock:
                if (
                    ...
                    sleep(add_jitter(friction_brake.fb_cfg.DECREASE_DELAY, friction_brake.cfg.BOT.JITTER))
                else:
                    cur_time = time()
